        ...
        jQuery(function() {
  var $hero = jQuery('#m-1593722414736');
  var $module = jQuery('#m-1593722414736').children('.module');
  ...
  if($module.attr('data-fixedMode') == '1'){
   ...
   changeImage();
    ...
    $module.parent().removeClass('gf_hero-fixed-mode');
        ...
        jQuery(function() {
  var mode = jQuery('.gryffeditor').hasClass('editing') ? 'dev' : 'production';
  var $module = jQuery('#m-1593722414654').children('.module');
  if (mode == 'dev') {
      jQuery('#m-1593722414654').attr('data-name', '').css('background-image', 'none').removeAttr('data-image');
      
      var flag = true;
      var $bkLiquid = parent.jQuery('body').find('#gfFrame').contents().find('#module-1593722414654');
      if ($bkLiquid && $bkLiquid.length > 0) {
          ...
          try {
              ...
              if (imageUrl != '') {
                  flag = false;
                  jQuery('#m-1593722414654').css('background-image', 'url(' + imageUrl + ')').css('min-height', '100px').attr('data-image', 'true');
              }
              if (name != '' && name != 'Custom Code') {
                  flag = false;
                  jQuery('#m-1593722414654').attr('data-name', name);
              }
          } catch(error) {
              console.log(error);
          }
      ...
      if (flag) {
          jQuery('#m-1593722414654').attr('data-name', 'Right click on the module, then choose Edit Html / Liquid option to start writing your custom code.');
      }
  }
});
        ...
        jQuery(function() {
  try {
    var $module = jQuery('#m-1593722414620').children('.module');
